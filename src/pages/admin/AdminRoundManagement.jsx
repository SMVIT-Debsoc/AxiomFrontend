...
export default function AdminRoundManagement() {
  ...
  useRoundSocket(roundId, {
    ...
    onRoundStatusChange: (data) => {
      console.log("[Socket] Round status update:", data);
      setRound((prev) =>
        prev ? {...prev, status: data.status, ...data} : prev
      );
    },
    onPairingsPublished: (data) => {
      console.log("[Socket] Pairings published update:", data);
      setRound((prev) =>
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
          {debates.length > 0 && (
            <button
              ...
              disabled={loading}
              className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed w-full md:w-auto ${
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-border gap-4 pb-0">
        <div className="flex overflow-x-auto pb-2 md:pb-0 no-scrollbar">
          <button
            ...
            className={`px-3 md:px-6 py-4 font-bold text-sm transition-all border-b-2 whitespace-nowrap ${
              activeTab === "checkins"
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span
                        className={`text-[10px] uppercase font-bold px-2 py-1 rounded flex-shrink-0 ${
                          status === "PRESENT"
                            ? "bg-green-500/10 text-green-500"
                            : "bg-red-500/10 text-red-500"
                        }`}
